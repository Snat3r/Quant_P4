...
#Replace all interpunction with " " so R can identify permissions with interpunction
PERM_QP4$permissions = gsub("[[:punct:]]", " ", QP4$permissions)
...
PERM_QP4$	make_receive_SIP_calls	<- str_extract(PERM_QP4$permissions, "receive SIP calls")
...
PERM_QP4$	read_TV_channel_program_information	<- str_extract(PERM_QP4$permissions, "read TV channel")
PERM_QP4$	write_TV_channel_program_information <- str_extract(PERM_QP4$permissions, "write TV channel")
...
PERM_QP4$	Google_Spreadsheets_Google_Docs	<- str_extract(PERM_QP4$permissions, "Google Spreadsheets")
...
PERM_QP4$	read_write_to_resources_owned_by_diag_record_what_you_type_and_actions_you_take	<- str_extract(PERM_QP4$permissions, "write to resources owned by diag record what you type and actions you take")
...
PERM_QP4$	change_intercept_network_settings_and_traffic	<- str_extract(PERM_QP4$permissions, "intercept network settings and traffic")
...
PERM_QP4$	approximate_location_network_based	<- str_extract(PERM_QP4$permissions, "approximate location")
PERM_QP4$	precise_location_GPS_and_network_based	<- str_extract(PERM_QP4$permissions, "precise location")
...
PERM_QP4$	modify_delete_internal_media_storage_contents	<- str_extract(PERM_QP4$permissions, "delete internal media storage contents")
...
PERM_QP4$	body_sensors_like_heart_rate_monitors	<- str_extract(PERM_QP4$permissions, "heart rate")
...
PERM_QP4[PERM_QP4 == "collapse status bar"] <- as.numeric(1)
...
PERM_QP4[PERM_QP4 == "receive SIP calls"] <- as.numeric(1)
...
PERM_QP4[PERM_QP4 == "read TV channel"] <- as.numeric(1)
PERM_QP4[PERM_QP4 == "write TV channel"] <- as.numeric(1)
...
PERM_QP4[PERM_QP4 == "Google Spreadsheets"] <- as.numeric(1)
...
PERM_QP4[PERM_QP4 == "write to resources owned by diag record what you type and actions you take"] <- as.numeric(1)
...
PERM_QP4[PERM_QP4 == "intercept network settings and traffic"] <- as.numeric(1)
...
PERM_QP4[PERM_QP4 == "approximate location"] <- as.numeric(1)
PERM_QP4[PERM_QP4 == "precise location"] <- as.numeric(1)
...
PERM_QP4[PERM_QP4 == "delete internal media storage contents"] <- as.numeric(1)
...
PERM_QP4[PERM_QP4 == "heart rate"] <- as.numeric(1)
...
#make subset with dangerous permissions
Dang_Perm <- c("id",
               "read_calendar_events_plus_confidential_information",
